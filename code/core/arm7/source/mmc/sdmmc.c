...
u32 _SDMMC_init(const u8 devNum)
{
...
u32 _SDMMC_setSleepMode(const u8 devNum, const bool enabled)
{
	if(devNum > SDMMC_MAX_DEV_NUM) return SDMMC_ERR_INVAL_PARAM;
...
u32 _SDMMC_deinit(const u8 devNum)
{
	if(devNum > SDMMC_MAX_DEV_NUM) return SDMMC_ERR_INVAL_PARAM;
...
u32 _SDMMC_lockUnlock(const u8 devNum, const u8 mode, const u8 *const pwd, const u8 pwdLen)
{
...
u32 _SDMMC_exportDevState(const u8 devNum, u8 devOut[64])
{
	if(devNum > SDMMC_MAX_DEV_NUM) return SDMMC_ERR_INVAL_PARAM;
...
u32 _SDMMC_importDevState(const u8 devNum, const u8 devIn[64])
{
	if(devNum > SDMMC_MAX_DEV_NUM) return SDMMC_ERR_INVAL_PARAM;
...
u32 _SDMMC_getDevInfo(const u8 devNum, SdmmcInfo *const infoOut)
{
	if(devNum > SDMMC_MAX_DEV_NUM) return SDMMC_ERR_INVAL_PARAM;
...
u32 _SDMMC_getCid(const u8 devNum, u32 cidOut[4])
{
	if(devNum > SDMMC_MAX_DEV_NUM) return SDMMC_ERR_INVAL_PARAM;
...
u32 _SDMMC_getSectors(const u8 devNum)
{
...
u32 _SDMMC_readSectors(const u8 devNum, u32 sect, void *const buf, const u16 count)
{
...
//       however the R1 card status may report ADDRESS_OUT_OF_RANGE on next(?) status read.
//       This error is normal for (e)MMC and can be ignored.
u32 _SDMMC_writeSectors(const u8 devNum, u32 sect, const void *const buf, const u16 count)
{
	if(devNum > SDMMC_MAX_DEV_NUM || count == 0) return SDMMC_ERR_INVAL_PARAM;
...
u32 _SDMMC_sendCommand(const u8 devNum, MmcCommand *const mmcCmd)
{
	if(devNum > SDMMC_MAX_DEV_NUM) return SDMMC_ERR_INVAL_PARAM;
...
u32 _SDMMC_getLastR1error(const u8 devNum)
{
	if(devNum > SDMMC_MAX_DEV_NUM) return 0;
